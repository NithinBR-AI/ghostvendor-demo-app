...
def send_email(to: str, subject: str, body: str, from_email: str = "noreply@ghostvendor.dev") -> int:
    ...
    try:
        ...
        return {"error": "timeout", "message": "SendGrid request timed out"}
    if response.status_code in (429, 502, 503):
        ...
        try:
            ...
            return {"error": "timeout", "message": "SendGrid request timed out on retry"}
        if response.status_code in (429, 502, 503):
            return {"error": "retry_failed", "message": f"SendGrid returned {response.status_code} after retry"}
