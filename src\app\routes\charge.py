...
def charge():
    ...
    try:
        ...
        logger.warning("Stripe create_payment_intent raised exception: %s", e)
        return jsonify({"error": "stripe_error", "message": str(e)}), 503
    if isinstance(result, dict) and "error" in result:
        logger.warning("Stripe create_payment_intent returned error: %s", result.get("message"))
        return jsonify(result), 503
    if not isinstance(result, dict) or "id" not in result or "status" not in result:
        logger.warning("Stripe create_payment_intent returned unexpected result: %s", result)
        return jsonify({"error": "stripe_error", "message": "Unexpected response from Stripe"}), 503
