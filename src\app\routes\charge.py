...
def charge():
    ...
    try:
        ...
        logging.warning("Stripe charge failed with exception: %s", e)
        return jsonify({"error": "vendor error"}), 503
    if isinstance(result, dict) and "error" in result:
        logging.warning("Stripe charge returned error: %s", result.get("error"))
        if result["error"] == "timeout":
            return jsonify({"error": "vendor error"}), 504
        elif result["error"] == "retry_failed":
            if "429" in result.get("message", ""):
                return jsonify({"error": "vendor error"}), 429
            return jsonify({"error": "vendor error"}), 502
        elif result["error"] == "malformed_response":
            return jsonify({"error": "vendor error"}), 502
        return jsonify({"error": "vendor error"}), 503
    if not isinstance(result, dict) or "id" not in result or "status" not in result:
        logging.warning("Stripe charge returned unexpected result: %s", result)
        return jsonify({"error": "vendor error"}), 502
