...
def charge():
    ...
    try:
        ...
        logger.warning("Stripe call failed with exception: %s", exc)
        return jsonify({"error": "vendor_error", "message": "Stripe request failed"}), 503
    if isinstance(result, dict) and "error" in result:
        logger.warning("Stripe returned error: %s", result)
        return jsonify({"error": result["error"], "message": result.get("message", "Stripe error")}), 503
    try:
        return jsonify({"payment_intent_id": result["id"], "status": result["status"]})
    except (KeyError, TypeError) as exc:
        logger.warning("Stripe response missing expected fields: %s", exc)
        return jsonify({"error": "vendor_error", "message": "Invalid response from Stripe"}), 503
