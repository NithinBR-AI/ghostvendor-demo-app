...
def charge():
    ...
    try:
        ...
        logger.warning("Stripe call failed with exception: %s", e)
        return jsonify({"error": "vendor error"}), 503
    if isinstance(result, dict) and "error" in result:
        logger.warning("Stripe returned error: %s", result.get("error"))
        return jsonify({"error": "vendor error"}), 503
    try:
        return jsonify({"payment_intent_id": result["id"], "status": result["status"]})
    except (KeyError, TypeError) as e:
        logger.warning("Malformed Stripe response: %s", e)
        return jsonify({"error": "vendor error"}), 503
