...
def charge():
    ...
    try:
        ...
        logger.warning("Stripe call raised exception: %s", e)
    ...
    if not isinstance(result, dict) or "id" not in result or "status" not in result:
        logger.warning("Stripe returned unexpected result: %s", result)
        return jsonify({"error": "vendor_error", "message": "Unexpected response from Stripe"}), 503
