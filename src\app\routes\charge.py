...
def charge():
    ...
    if not isinstance(result, dict) or "error" in result:
        logger.warning("Stripe returned error payload: %s", result)
        return jsonify({"error": "vendor error", "details": result}), 503
    try:
        return jsonify({"payment_intent_id": result["id"], "status": result["status"]})
    except KeyError as exc:
        logger.warning("Stripe response missing expected key: %s", exc)
        return jsonify({"error": "vendor error", "message": "unexpected response format"}), 503
