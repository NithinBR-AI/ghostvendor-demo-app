...
def charge():
    ...
    if isinstance(result, dict) and "error" in result:
        logger.warning("Stripe returned error: %s", result.get("message", result["error"]))
        return jsonify({"error": "vendor error", "message": result.get("message", result["error"])}), 503
    if not isinstance(result, dict) or "id" not in result or "status" not in result:
        ...
        return jsonify({"error": "vendor error", "message": "unexpected response from Stripe"}), 503
