...
def charge():
    ...
    try:
        ...
        logger.warning("Stripe client exception: %s", exc)
        return jsonify({"error": "vendor error", "message": str(exc)}), 503
    if isinstance(result, dict) and "error" in result:
        logger.warning("Stripe returned error: %s", result["error"])
    ...
    try:
        ...
        logger.warning("Unexpected Stripe response format: %s", exc)
        return jsonify({"error": "vendor error", "message": "Unexpected response format from Stripe"}), 503
