...
def notify():
    ...
    try:
        ...
        logger.warning("SendGrid call raised exception: %s", e)
        return jsonify({"error": "vendor error", "message": str(e)}), 503
    if isinstance(result, dict) and "error" in result:
        logger.warning("SendGrid returned error: %s", result)
        return jsonify(result), 503
    if isinstance(result, int) and result >= 400:
        logger.warning("SendGrid returned non-2xx status: %d", result)
        return jsonify({"error": "vendor error", "message": f"SendGrid returned {result}"}), 503
